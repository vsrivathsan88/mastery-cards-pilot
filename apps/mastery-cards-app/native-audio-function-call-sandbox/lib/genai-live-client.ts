...
export class GenAILiveClient extends EventEmitter<LiveClientEventTypes> {
  ...
  // Track transcriptions for current turn to emit as final when turnComplete is received
  ...
  protected onMessage(message: LiveServerMessage) {
    ...
    if (message.serverContent) {
      ...
      if (serverContent.interrupted) {
        ...
        // Clear accumulated transcripts on interruption
      ...
      if (serverContent.inputTranscription) {
        ...
        // Accumulate transcript for the current turn
        this.currentInputTranscript += text;

        // Emit as interim (for real-time display)
      ...
      if (serverContent.outputTranscription) {
        ...
        // Emit as interim (for real-time display)
        this.emit('outputTranscription', text, false);
      ...
      if (serverContent.turnComplete) {
        ...
        // Emit final transcripts when turn is complete
        if (this.currentInputTranscript.trim()) {
          this.emit('inputTranscription', this.currentInputTranscript, true);
          this.log('server.inputTranscription (final)', this.currentInputTranscript);
          this.currentInputTranscript = ''; // Clear for next turn
        }

        if (this.currentOutputTranscript.trim()) {
          this.emit('outputTranscription', this.currentOutputTranscript, true);
          this.log('server.outputTranscription (final)', this.currentOutputTranscript);
          this.currentOutputTranscript = ''; // Clear for next turn
        }
  ...
  protected onClose(e: CloseEvent) {
    ...
    // Clear accumulated transcripts on disconnect
    this.currentInputTranscript = '';
    this.currentOutputTranscript = '';
